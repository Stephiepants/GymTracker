...
export class RendererCanvas2d {
  ...
  drawKeypoints(keypoints) {
    ...
    const keypointCoordinatesLeftKneeHipShoulder =
      keypointIndicesLeftKneeHipShoulder.map((i) => ({
        x: keypoints[i].x,
        y: keypoints[i].y,
      }));
    ...
    const keypointCoordinatesRightKneeHipShoulder =
    ...
    function updateAngleAndDisplay(keypointCoordinates, elementId) {
      if (keypointCoordinates.length === 3) {
        const angle = this.calculateAngle(
          keypointCoordinates[0],
          keypointCoordinates[1],
          keypointCoordinates[2]
        );
    ...
    if (keypointCoordinatesRightArm.length === 3) {
      this.angleRightArm = this.calculateAngle(
        keypointCoordinatesRightArm[0],
        keypointCoordinatesRightArm[1],
        keypointCoordinatesRightArm[2]
      );
      // Update the angle value in the HTML element
    ...
    /* if (keypointCoordinatesLeftKneeHipShoulder.length === 3) {
      const angle = this.calculateAngle(
        ...
        keypointCoordinatesLeftKneeHipShoulder[2]
      );
      // Update the angle value in the HTML element
      const angleValueElement = document.getElementById(
    ...
    /* if (keypointCoordinatesRightKneeHipShoulder.length === 3) {
      const angle = this.calculateAngle(
        ...
        keypointCoordinatesRightKneeHipShoulder[2]
      );
      // Update the angle value in the HTML element
      const angleValueElement = document.getElementById(
...
document.querySelector("#connect").addEventListener("click", function () {
  if (BLE_forcePlates.isWebBluetoothEnabled()) {
    BLE_forcePlates.onConnectButtonClick();
  }
});

document.querySelector("#start").addEventListener("click", function (event) {
  if (BLE_forcePlates.isWebBluetoothEnabled()) {
    BLE_forcePlates.onStartButtonClick();
  }
});
