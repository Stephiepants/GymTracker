...
var battery_levelReadNotifyCharacteristic = '00002a19-0000-1000-8000-00805f9b34fb'
...
try {
  ...
  const device = await navigator.bluetooth.requestDevice(
    {
    filters: [{ services: [ForcePlateUUIDS.PRIMARY_SERVICE] }, { namePrefix: "ForcePlate" }],
  ...
  const notifyCharacteristic = await service.getCharacteristic(ForcePlateUUIDS.NOTIFY);
  ...
  const writeCharacteristic = await service.getCharacteristic(ForcePlateUUIDS.WRITE);
...
export function onStopButtonClick() {
  if (acquiredNotifyGATTCharacteristic) {
    acquiredNotifyGATTCharacteristic.stopNotifications()
    .then(() => {
      console.log('> Notifications stopped');
      acquiredNotifyGATTCharacteristic.removeEventListener('characteristicvaluechanged',
          handleNotification);
    document.querySelector('#start').disabled = false
    document.querySelector('#stop').disabled = true
    })
  ...
  try {
    ...
    acquiredNotifyGATTCharacteristic.addEventListener('characteristicvaluechanged', handleNotification);
    document.querySelector('#start').disabled = true;
  ...
  if (acquiredWriteGATTCharacteristic) {
    ...
    acquiredWriteGATTCharacteristic.writeValue(valueToWrite)
      .then(() => {
...
function handleNotification(event) {
  ...
  if (event.currentTarget.service.device.name == "ForcePlate0011") {
    ...
    parseSensorData("ForcePlate0011", event);
  } else {
    //console.log('Received notification event forceplate 0010:', event);
    parseSensorData("ForcePlate0010", event);
  }
}
